...
use fretnet_runtime::{
    default_existing_feature_fixture_paths, default_model_path, load_feature_reference_fixture, load_regression_fixture,
    FeatureExtractor, FrontendConfig, FretNetRuntime,
};
...
fn rust_frontend_runs_end_to_end_into_onnx_runtime() {
    ...
    if !model_path.exists() {
        eprintln!("skipping end-to-end frontend test because model is missing at {}", model_path.display());
        return;
    }

    let fixture_paths = default_existing_feature_fixture_paths();
    if fixture_paths.is_empty() {
        eprintln!("skipping end-to-end frontend test because no Python feature fixtures are present");
        return;
    }
    ...
    for feature_fixture_path in fixture_paths {
        let feature_fixture = load_feature_reference_fixture(&feature_fixture_path).expect("feature fixture should load");
        ...
        let output = runtime.infer_features(&batch).expect("inference should succeed");

        println!("Fixture: {feature_name}");
        println!("  HCQT shape: {:?}", hcqt.shape());
        println!("  Model input shape: {:?}", batch.shape());
        println!("  Output tensors: {:?}", output.tensors.iter().map(|tensor| tensor.name.clone()).collect::<Vec<_>>());
        ...
        for tensor in &output.tensors {
            assert!(tensor.data.iter().all(|value| value.is_finite()), "tensor '{}' contains non-finite values", tensor.name);
        }

        if let Some(regression_path) = regression_fixture_for_feature_fixture(&feature_fixture_path) {
            let regression_fixture = load_regression_fixture(&regression_path).expect("regression fixture should load");
            assert_eq!(
